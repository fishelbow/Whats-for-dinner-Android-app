...
fun addItemByName(name: String, quantity: String) = viewModelScope.launch {
    val cleanedQuantity = quantity.toIntOrNull()?.takeIf { it > 0 }?.toString() ?: "1"
    val listId = currentListId.value ?: return@launch
  ...
  //  if (pantryItem.quantity > 0) return@launch
    ...
    val existingItem = shoppingItems.value.firstOrNull {
        it.pantryItemId == pantryItem.id && it.listId == listId
    }

    val addedQty = cleanedQuantity.toDoubleOrNull() ?: return@launch

    if (existingItem != null) {
        val currentQty = existingItem.quantity.toDoubleOrNull() ?: 0.0
        val newQty = formatQuantity(currentQty + addedQty)
